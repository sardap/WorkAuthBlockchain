...
namespace prototype
{
    class Program
    {
		...
		static readonly HexBigInteger GASLIMT = new HexBigInteger(3000000);
		...
		static async Task<string> PublishWorkHistoryAsync(string data, string senderAddress, string senderPassword, string targetAddress)
		{
			Web3 web3 = new Web3();

			bool unlockAcountResult = await web3.Personal.UnlockAccount.SendRequestAsync(senderAddress, senderPassword, LOGIN_TIMEOUT);

			string encryptedData = EncyptData(data, targetAddress);

			string trasnactionHash = await web3.Eth.DeployContract.SendRequestAsync(abi, BYTE_CODE, senderAddress, GASLIMT, encryptedData, data.GetHashCode());
